...
class GrammarTests(unittest.TestCase):
    ...
    def test_var_annot_module_semantics(self):
        ...
        self.assertEqual(ann_module.__annotations__, {1: 2, "x": int, "y": str, "f": typing.Tuple[int, int]})
    ...
    def test_listcomps(self):
        ...
        self.assertEqual(
            [(lambda a: [a**i for i in range(a + 1)])(j) for j in range(5)], [[1], [1, 1], [1, 2, 4], [1, 3, 9, 27], [1, 4, 16, 64, 256]]
        )
        ...
        x = [
            (sname, pname) for (sno, sname) in suppliers for (pno, pname) in parts for (sp_sno, sp_pno) in suppart if sno == sp_sno and pno == sp_pno
        ]
