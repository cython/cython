...
class ExceptionTests(unittest.TestCase):
    ...
    @unittest.skipIf(sys.version_info < (3,11), "needs Python 3.11")
    def test_notes(self):
    ...
    def test_generator_leaking(self):
        ...
        self.assertIsNone(sys_exception())
        self.assertIsInstance(next(g), KeyError)
        self.assertIsNone(sys_exception())
        ...
        try:
            ...
            g = yield_raise()
            self.assertIsInstance(next(g), KeyError)
            self.assertIsInstance(sys_exception(), TypeError)
            self.assertIsInstance(next(g), KeyError)
            self.assertIsInstance(sys_exception(), TypeError)
    ...
    def test_generator_doesnt_retain_old_exc(self):
        def g():
            self.assertIsInstance(sys_exception(), RuntimeError)
            yield
            self.assertIsNone(sys_exception())
        it = g()
        try:
    ...
    @unittest.skip("currently crashes in Cython")
    ...
    @unittest.skipIf(getattr(support, 'Py_TRACE_REFS', False), 'cannot test Py_TRACE_REFS build')
    ...
    @unittest.skip("not relevant for Cython - and requires 'support.catch_unraisable_exception()')")
...
class ImportErrorTests(unittest.TestCase):
    ...
    def test_attributes(self):
        ...
        if sys.version_info >= (3,13):
            ...
            msg = r"'invalid' is an invalid keyword argument for ImportError()"
