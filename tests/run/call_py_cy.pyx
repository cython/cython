...
def cy_call_ordering_callee(a, b, **kwds):
    c = kwds['c']
    d = kwds['d']
    e = kwds['e']
    return a, b, c, d, e

def cy_call_ordering():
    """
    ...
    for len_ in range(2):
        ...
        for perm in permutations(list(zip(keys[len_:], values[len_:]))):
            ...
            res = eval('cy_call_ordering_callee(%s %s)' % (star_str, starstar_str))
            assert res==tuple(values)
            res = eval('cy_call_ordering_callee(*star, %s)' % starstar_str)
            assert res==tuple(values)
        res = eval('cy_call_ordering_callee(%s **starstar)' % star_str)
        assert res==tuple(values)
        res = eval('cy_call_ordering_callee(*star, **starstar)')
