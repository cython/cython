...
- **Constructor optimization supports optional args and cross-module vtable dispatch** (`OptimizeExtTypeConstructorCalls._maybe_split_ctor`, `_build_init_call_stat`, `Optimize.py:~4727`). Key design points:
  1. Same-module: `func_cname` (`__pyx_f_…`) is declared in the same TU → `PythonCapiCallNode(func_cname, …)`.
  2. Cross-module: `func_cname` is NOT declared in the calling TU (cimport only imports the vtable pointer). Use `ext_type.vtabptr_cname->init_entry.cname(…)` instead. `has_vtable` flag controls which path is taken.
  3. When some optional args are passed positionally, `call.has_optional_args = True` activates the `__pyx_opt_args_*` struct machinery in `generate_result_code`.
  4. `call_type` uses `is_overridable=True` so `c_call_code` inserts `__pyx_skip_dispatch` before the opt-args struct pointer (the correct ABI order). `wrapper_call=True` fixes the skip-dispatch value to 1.
  5. Coverage: `tests/run/auto_cpdef_optargs_methods.py`, `tests/build/embed_modules_optimize.srctree`.
