...
- **Constructor split path must not fire for module/Python-class-body LHS** (`OptimizeExtTypeConstructorCalls._maybe_split_ctor`, `Optimize.py:~4848`): `_maybe_split_ctor` uses `lhs.entry.cname` directly as a C lvalue in the generated code. This is only valid when the LHS lives in a `LocalScope` (function body), where Cython declares `PyObject *__pyx_v_name = 0;` at the top. Module-level variables (`is_module_scope`) and Python class body variables (`is_py_class_scope` — Enum members, NamedTuple fields, etc.) live in dicts; no C local is ever declared for them, so the generated `__pyx_v_9module_name_varname` produces an "undeclared identifier" clang error. Fix: early-return in `_maybe_split_ctor` when `lhs.entry.scope.is_module_scope or lhs.entry.scope.is_py_class_scope`. This falls through to `_maybe_transform_ctor_expr` (expression-level path via `TempResultFromStatNode`), which handles all contexts correctly.
...
- **`typing.final` / `typing_extensions.final` get the same semantics as `cython.final`** (`ParseTreeTransforms.py`, `Builtin.py`, `Nodes.py`). Two import forms: **name form** (`from typing import final; @final`, classes AND methods) is registered in `InterpretCompilerDirectives.visit_FromImportStatNode`/`visit_FromCImportStatNode` — when the module is `typing`/`typing_extensions` and the imported name is `final`, `directive_names[local_name] = 'final'` (import left in place). This consumes the decorator early and flows the `final` directive uniformly to class (`Symtab.py:1902`) and method (`Symtab.py:2773`) scopes. **Attribute form** (`import typing; @typing.final`, classes only) goes through `Builtin.exprnode_to_known_standard_library_name`: `final` is now declared in the `typing` and a new `typing_extensions` known-module scope (`get_known_standard_library_module_scope`), and `Nodes.CClassDefNode._handle_cclass_decorators` (~6168) maps `typing.final`/`typing_extensions.final` → `extra_directives["final"]=True`. **Gotcha:** `is_final_type` is read inside `declare_c_class` (`Symtab.py:1902`) from the *enclosing* `env.directives` — which runs BEFORE `extra_directives` is merged into the new `scope.directives` (`Nodes.py:~6300`). So the attribute-form `final` is applied directly via `if extra_directives.get('final'): self.entry.type.is_final_type = True` right after the entry is created (`Nodes.py:~6290`), before method declarations. Method attribute-form is intentionally unsupported. **Leniency rule — `typing.final` is ADVISORY, `cython.final` is STRICT** (crop-chronicles hit both: `from typing import final` on a plain `class Config(ConfigBase)`, and on a `cpdef` method of a non-final class). Unlike `@cython.final` (which errors when it can't apply), `typing.final` must be a silent no-op where it can't apply, never an error. Two cases: (1) **plain Python `class` scope** — `final`'s legal scopes are `('cclass','function')`, so on a plain `class` it would raise "final not allowed in class scope"; (2) **method of a non-final class** — would raise "Only final types can have final Python (def/cpdef) methods" (`Nodes.py:4032`, gated on `is_final_cmethod` set in `Symtab.py:2773`). Mechanism: local names bound to typing's `final` are tracked in `InterpretCompilerDirectives._typing_final_names`. In `_extract_directives`: if the scope is one where `final` is illegal (plain class) the decorator is dropped silently; otherwise its directive value is rewritten from `True` to the `Options.FINAL_ADVISORY` sentinel (a truthy string, so the class-level `if directives.get('final')` checks at `Symtab.py:1902`/`Nodes.py:6297` still apply final to a cclass unchanged). `Symtab.CClassScope.declare_cfunction` (`:2773`) treats `final == FINAL_ADVISORY` as effective only when `parent_type.is_final_type` — so an advisory final method on a non-final class is a no-op, while strict `cython.final` (value `True`) still flows to the `:4032` error. Coverage: `tests/run/typing_final.pyx` (`PlainFinal` + `NonFinalBase.mtd` no-ops), `tests/run/typing_final_attr.pyx`. (There is intentionally NO `typing.final` error test — it never errors.)
- **`@dataclasses.dataclass`/`@total_ordering` promote a Python class to cclass in the directive prescan** (`ParseTreeTransforms._extract_directives`, ~1375). `@final @dc(frozen=True) class X` (cython.dataclasses, no explicit `@cclass`) used to error *"The final compiler directive is not allowed in class scope"*: the prescan that upgrades `scope_name` 'class'→'cclass' (so scope-restricted directives like `@final`/`@python_subclassing` pass regardless of decorator order) only triggered on a `cclass` directive. It now triggers on any of `('cclass', 'dataclasses.dataclass', 'total_ordering')` — all of which `AutoCpdefTransform.converts_to_cclass` turns into a cclass. Stdlib `@dataclasses.dataclass`+`@cython.cclass` already routes to the same `handle_cclass_dataclass` path (via `_handle_cclass_decorators` known-stdlib detection); the only delta from `@cython.dataclasses.dataclass` is a dead runtime `from dataclasses import dataclass` import, left as-is.
- **Dataclass cpdef `__init__` lowers required C-typed fields to their C type** (`Dataclass.py:handle_cclass_dataclass`, ~400). The arg re-typing loop (compensating for the `annotation_typing=False` wrapper) used to skip required args (`if arg.default is None: continue`), so `row: cython.int` became `PyObject *row` in the slot. Now every arg whose field has a non-pyobject type gets `arg.type = field_entry.type`, EXCEPT optional args backed by `default_factory` (must stay `PyObject *` for the `_HAS_DEFAULT_FACTORY` sentinel). Fixes both `@dataclasses.dataclass` and `@cython.dataclasses.dataclass` identically. **Companion fix (pre-existing latent bug):** the forced `c_returncode_type` return needs an exception value or the error epilogue can't propagate, producing a spurious *"Unraisable exception in function '__init__'"* warning (caught by `tests/errors/dataclass_e5.pyx`, `dataclass_w1.pyx`). Set `cfunc.declarator.exception_value = ExprNodes.IntNode.for_int(pos, -1, type=c_returncode_type)` (the same `except -1` convention property setters use); then `err_val` is `-1` not `None`, so the warning at `Nodes.py:2461` no longer fires. Coverage: `tests/run/cdef_class_dataclass.pyx` (`TypedRequiredFields`/`StdlibTypedRequired`), `tests/build/embed_modules_optimize.srctree` (`DataClass2`).
- **Exception spec as first-class cpdef override signature** (`PyrexTypes.py`, `Nodes.py`, `Symtab.py`, `ParseTreeTransforms.py`): `CFuncType` now carries `has_explicit_exc_clause` (class default `False`, constructor keyword). Wired from `CFuncDeclaratorNode.analyse` (~868) after the two clearing sites (`Nodes.py:786/807`). `with_with_gil` and `specialize` both preserve it. Three new methods on `CFuncType`: `exception_spec_string()` (human-readable spec); `compatible_cmethod_override_with(base_type)` (returns False when override widens base's noexcept promise, or changes sentinel value); `with_exception_spec_of(base_type)` (returns copy with base's exception spec + `has_explicit_exc_clause`). In `CClassScope.declare_cfunction`: (1) before the signature cascade, inherit base's exception spec into unannotated overrides (`entry.type.is_overridable and not type.has_explicit_exc_clause` and not `__init__`/static/classmethod); (2) in the else branch, emit a specific "Exception specification of … is wider" error when the mismatch is in the exception facet. `declare_cproperty` accepts `has_explicit_exc_clause` param and forwards it to the CFuncType. `_convert_property_to_cpdef` reads `@cython.exceptval` directive and sets `has_explicit_exc_clause=False` when absent (so subclass getters can inherit). **`entry.is_overridable` is always 0 for CClass method entries — use `entry.type.is_overridable` instead.** Coverage: `tests/run/cpdef_exc_spec_inheritance.pyx`, `tests/run/cpdef_exc_spec_cross_module.srctree`, `tests/errors/cpdef_exc_spec_override_conflict.pyx`.
