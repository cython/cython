...
- **Constructor optimization for `@dataclass` cdef classes** (`Dataclass.py:generate_init_code`, `Symtab.py:CClassScope.declare_pyfunction`): `generate_init_code` emits `cpdef __init__` (instead of `def`) when `kw_only=False`. This gives the entry `is_cmethod=True` so `OptimizeExtTypeConstructorCalls._extract_ctor_info` fires. Two companion fixes needed: (1) After `analyse_decs_transform.visit(comp_directives)`, call `node.entry.scope.allocate_vtable_names(node.entry)` in `handle_cclass_dataclass` — `allocate_vtable_names` ran before the cpdef was added to `cfunc_entries`, so vtabptr_cname would be None, producing `None->__pyx___init__` in the `__pyx_pf_` body for optional-arg cpdef methods. (2) `CClassScope.declare_pyfunction` was generating a spurious level-0 `'<name>' redeclared` warning (visible at `runtests.py`'s `LEVEL=0`) when `declare_cpdef_wrapper` created the Python wrapper — fix: use `visibility='ignore'` when `not allow_redefine` (wrapper case) and an existing cfunction entry is found. `kw_only=True` dataclasses stay as `def __init__` (kw-only args are not cpdef-compatible). Coverage: `tests/run/cdef_class_dataclass.pyx` (`make_dataclass_ctor_opt`, `make_dataclass_ctor_opt_expr` with `@cython.test_fail_if_path_exists("//SimpleCallNode")`).
