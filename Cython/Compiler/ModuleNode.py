...
def generate_c_code_config(env, options):
    if Options.options_in_thread.annotate or options.annotate:
        emit_linenums = False
    else:
...
class ModuleNode(Nodes.Node, Nodes.BlockNode):
    ...
    def analyse_declarations(self, env):
        ...
        elif Options.options_in_thread.embed_pos_in_docstring:
    ...
    def generate_c_code(self, env, options, result):
        ...
        if Options.options_in_thread.annotate or options.annotate:
            show_entire_c_code = Options.options_in_thread.annotate == "fullc" or options.annotate == "fullc"
        ...
        if Options.options_in_thread.embed:
        ...
        if (
            (Options.options_in_thread.annotate or options.annotate) and not
    ...
    def _generate_annotations(self, rootwriter, result, options):
        ...
        coverage_xml_filename = Options.options_in_thread.annotate_coverage_xml or options.annotate_coverage_xml
    ...
    def generate_module_preamble(self, env, options, cimported_modules, metadata, code):
        ...
        if Options.options_in_thread.pre_import is not None:
    ...
    def generate_cfunction_declarations(self, env, code, definition):
        for entry in env.cfunc_entries:
            from_pyx = Options.options_in_thread.cimport_from_pyx and not entry.visibility == 'extern'
            if (entry.used
    ...
    def generate_clear_function(self, scope, code, cclass_entry):
        ...
        if py_attrs and Options.options_in_thread.clear_to_none:
        ...
        if Options.options_in_thread.clear_to_none:
            for entry in py_attrs:
    ...
    def generate_module_cleanup_func(self, env, code):
        ...
        if Options.options_in_thread.generate_cleanup_code >= 2:
        ...
        if Options.options_in_thread.generate_cleanup_code >= 3:
...
#                    code.entry_as_pyobject(entry), entry.cname))
    ...
    def generate_main_method(self, env, code):
        module_is_main = self.is_main_module_flag_cname()
        if Options.options_in_thread.embed == "main":
            wmain = "wmain"
        else:
            wmain = Options.options_in_thread.embed
        main_method = TempitaUtilityCode.load_cached(
                "MainFunction", "Embed.c",
                context={
                    'module_name': env.module_name,
                    'module_is_main': module_is_main,
                    'main_method': Options.options_in_thread.embed,
                    'wmain_method': wmain,
                    'embed_modules': tuple(Options.options_in_thread.embed_modules)})
        ...
        if Options.options_in_thread.pre_import is not None:
            code.putln(
                '%s = __Pyx_PyImport_AddModuleRef("%s"); %s' % (
                    ...
                    Options.options_in_thread.pre_import,
    ...
    def _select_exported_entries(self, all_entries):
        return [
            ...
            if entry.api or entry.defined_in_pxd or (Options.options_in_thread.cimport_from_pyx and entry.visibility != 'extern')
        ]
