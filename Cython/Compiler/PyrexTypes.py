...
class BaseType:
    ...
    def cast_code(self, expr_code):
        return f"(({self.empty_declaration_code()}){expr_code})"
    ...
    def specialization_name(self):
        if self._specialization_name is None:
            ...
            self._specialization_name = re.sub(
                '[^a-zA-Z0-9_]', lambda x: f'_{ord(x.group(0)):x}_', common_subs)
        return self._specialization_name

    def base_declaration_code(self, base_code, entity_code):
        if entity_code:
            return f"{base_code} {entity_code}"
        else:
            return base_code
...
class PyrexType(BaseType):
    ...
    def _assign_from_py_code(self, source_code, result_code, error_pos, code,
        ...
        args = ', ' + ', '.join(f'{arg}' for arg in extra_args) if extra_args else ''
        convert_call = "{}({}{})".format(
        ...
        if special_none_cvalue:
            ...
            convert_call = "(__Pyx_Py_IsNone({}) ? ({}) : ({}))".format(
    ...
    def cpp_optional_declaration_code(self, entity_code, dll_linkage=None):
        ...
        raise NotImplementedError(
            f"cpp_optional_declaration_code only implemented for c++ classes and not type {self}")


def public_decl(base_code, dll_linkage):
    if dll_linkage:
        return f"{dll_linkage}({base_code.replace(',', ' __PYX_COMMA ')})"
    else:
        return base_code
...
class CTypedefType(BaseType):
    ...
    def declaration_code(self, entity_code,
        ...
        if self.typedef_namespace is not None and not pyrex:
            base_code = f"{self.typedef_namespace.empty_declaration_code()}::{base_code}"
    ...
    def __repr__(self):
        return f"<CTypedefType {self.typedef_cname}>"
        ...
        if self.typedef_is_external:
            if not self.to_py_utility_code:
                ...
                if type(base_type) is CIntType:
                    ...
                    cname = f"__pyx_convert_PyObject_string_to_py_{type_identifier(self)}"
                    context = {
                        'cname': cname,
                        'type': self.typedef_cname,
                    }
        ...
        if self.typedef_is_external:
            if not self.from_py_utility_code:
                ...
                if type(base_type) is CIntType:
                    ...
                    cname = f'__pyx_convert_string_from_py_{type_identifier(self)}'
                    context = {
                        'cname': cname,
                        'type': self.typedef_cname,
                    }
    ...
    def overflow_check_binop(self, binop, env, const_rhs=False):
        ...
        return f"__Pyx_{binop}_{name}_checking_overflow"
...
class MemoryViewSliceType(PyrexType):
    ...
    def validate_memslice_dtype(self, pos):
        if not self.valid_dtype(self.dtype):
            error(pos, f"Invalid base type for memoryview slice: {self.dtype}")
    ...
    def specialization_name(self):
        return f'{super().specialization_name()}_{self.specialization_suffix()}'

    def specialization_suffix(self):
        return f"{self.axes_to_name()}_{self.dtype_name}"
    ...
    def dtype_object_conversion_funcs(self, env):
        get_function = f"__pyx_memview_get_{self.dtype_name}"
        set_function = f"__pyx_memview_set_{self.dtype_name}"
    ...
    def axes_to_code(self):
        ...
        return [f"({d[a]} | {d[p]})" for a, p in self.axes]

    def axes_to_name(self):
        """Return an abbreviated name for our axes"""
        from . import MemoryView
        d = MemoryView._spec_to_abbrev
        return "".join([f"{d[a]}{d[p]}" for a, p in self.axes])

    def error_condition(self, result_code):
        return f"!{result_code}.memview"
    ...
    def __str__(self):
        ...
        return f"{dtype_name}[{', '.join(axes_code_list)}]"
    ...
    def generate_xdecref_clear(self, code, cname, clear_before_decref, **kwds):
        ...
        code.putln(f"{cname}.memview = NULL; {cname}.data = NULL;")
...
class BufferType(BaseType):
    ...
    def __repr__(self):
        return f"<BufferType {self.base!r}>"
...
class PyObjectType(PyrexType):
    ...
    def generate_incref(self, code, cname, nanny):
        if nanny:
            ...
            code.putln(f"__Pyx_INCREF({self.as_pyobject(cname)});")
        else:
            code.putln(f"Py_INCREF({self.as_pyobject(cname)});")

    def generate_xincref(self, code, cname, nanny):
        if nanny:
            code.funcstate.needs_refnanny = True
            code.putln(f"__Pyx_XINCREF({self.as_pyobject(cname)});")
        else:
            code.putln(f"Py_XINCREF({self.as_pyobject(cname)});")
    ...
    def generate_gotref(self, code, cname):
        code.funcstate.needs_refnanny = True
        code.putln(f"__Pyx_GOTREF({self.as_pyobject(cname)});")

    def generate_xgotref(self, code, cname):
        code.funcstate.needs_refnanny = True
        code.putln(f"__Pyx_XGOTREF({self.as_pyobject(cname)});")

    def generate_giveref(self, code, cname):
        code.funcstate.needs_refnanny = True
        code.putln(f"__Pyx_GIVEREF({self.as_pyobject(cname)});")

    def generate_xgiveref(self, code, cname):
        code.funcstate.needs_refnanny = True
        code.putln(f"__Pyx_XGIVEREF({self.as_pyobject(cname)});")

    def generate_decref_set(self, code, cname, rhs_cname):
        code.funcstate.needs_refnanny = True
        code.putln(f"__Pyx_DECREF_SET({cname}, {rhs_cname});")

    def generate_xdecref_set(self, code, cname, rhs_cname):
        code.funcstate.needs_refnanny = True
        code.putln(f"__Pyx_XDECREF_SET({cname}, {rhs_cname});")
    ...
    def _generate_decref(self, code, cname, nanny, null_check=False,
        ...
        if clear:
            if clear_before_decref:
                ...
                code.putln(f"{prefix}_{X}CLEAR({cname});")
            else:
                code.putln("{}_{}DECREF({}); {} = 0;".format(
                    prefix, X, self.as_pyobject(cname), cname))
        else:
            code.putln(f"{prefix}_{X}DECREF({self.as_pyobject(cname)});")
...
class BuiltinObjectType(PyObjectType):
    ...
    def isinstance_code(self, arg):
        return f'{self.type_check_function(exact=False)}({arg})'
    ...
    def type_test_code(self, arg, notnone=False, exact=True):
        ...
        if self.name == 'basestring':
            ...
            name = f'"{self.name}"'
        return check + f' || __Pyx_RaiseUnexpectedTypeError({name}, {arg})'
...
class PyExtensionType(PyObjectType):
    ...
    def declaration_code(self, entity_code,
        ...
        if pyrex or for_display:
            ...
            if self.typedef_flag:
                ...
                objstruct = f"struct {self.objstruct_cname}"
    ...
    def __repr__(self):
        return "<PyExtensionType {}{}>".format(self.scope.class_name,
    ...
    def py_type_name(self):
        ...
        return f"__import__({self.module_name!r}, None, None, ['']).{self.name}"
...
class CType(PyrexType):
    ...
    def error_condition(self, result_code):
        ...
        if self.is_string or self.is_pyunicode_ptr:
            conds.append(f"(!{result_code})")
        elif self.exception_value is not None:
            conds.append(f"({result_code} == ({self.sign_and_name()}){self.exception_value})")
        if self.exception_check:
    ...
    def to_py_call_code(self, source_code, result_code, result_type, to_py_function=None):
        ...
        return f"{result_code} = {func}({source_code or 'NULL'})"
...
class PythranExpr(CType):
    ...
    def __init__(self, pythran_type, org_buffer=None):
        ...
        self.from_py_function = f"from_python<{self.pythran_type}>"
        self.scope = None

    def declaration_code(self, entity_code, for_display=0, dll_linkage=None, pyrex=0):
        assert not pyrex
        return f"{self.cname} {entity_code}"
...
class CConstOrVolatileType(BaseType):
    ...
    def __repr__(self):
        return f"<CConstOrVolatileType {self.cv_string()}{self.cv_base_type!r}>"
...
class FusedType(CType):
    ...
    def __repr__(self):
        return f'FusedType(name={self.name!r})'
...
class CNumericType(CType):
    ...
    def __repr__(self):
        return f"<CNumericType {self.sign_and_name()}>"
    ...
    def get_to_py_type_conversion(self):
        if self.rank < list(rank_to_type_name).index('int'):
            ...
            if self.rank >= list(rank_to_type_name).index('PY_LONG_LONG'):
                Prefix = "Long"
                TypeName = "LongLong"
            return f"Py{Prefix}_From{SignWord}{TypeName}"
    ...
    def overflow_check_binop(self, binop, env, const_rhs=False):
        ...
        if binop == "lshift":
            ...
            if type in ('int', 'long', 'long long'):
                ...
                return f"__Pyx_{binop}_{name}_no_overflow"
            else:
                _load_overflow_base(env)
                env.use_utility_code(TempitaUtilityCode.load_cached(
                    "SizeCheck", "Overflow.c",
...
class CReturnCodeType(CIntType):
    ...
    def convert_to_pystring(self, cvalue, code, format_spec=None):
        return f"__Pyx_NewRef({code.globalstate.get_py_string_const(StringEncoding.EncodedString('None')).cname})"
...
class CBIntType(CIntType):
    ...
    def convert_to_pystring(self, cvalue, code, format_spec=None):
        ...
        code.globalstate.use_utility_code(to_pyunicode_utility)
        return f"{utility_code_name}({cvalue})"
...
class CComplexType(CNumericType):
    ...
    def to_py_function(self):
        return f"__pyx_PyComplex_FromComplex{self.implementation_suffix}"

    def __init__(self, real_type):
        while real_type.is_typedef and not real_type.typedef_is_external:
            real_type = real_type.typedef_base_type
        self.funcsuffix = f"_{real_type.specialization_name()}"
        ...
        self.binops = {}
        self.from_parts = f"{self.specialization_name()}_from_parts"
        self.default_value = f"{self.from_parts}(0, 0)"
    ...
    def attributes_known(self):
        if self.scope is None:
            ...
            scope.declare_cfunction(
                    ...
                    pos=None,
                    defining=1,
                    cname=f"__Pyx_c_conj{self.funcsuffix}")
    ...
    def lookup_op(self, nargs, op):
        ...
        try:
            ...
            self.binops[nargs, op] = func_name = f"__Pyx_c_{op_name}{self.funcsuffix}"
            return func_name
        except KeyError:
            return None
    ...
    def real_code(self, expr_code):
        return f"__Pyx_CREAL{self.implementation_suffix}({expr_code})"

    def imag_code(self, expr_code):
        return f"__Pyx_CIMAG{self.implementation_suffix}({expr_code})"
...
class SoftCComplexType(CComplexType):
    ...
    def __repr__(self):
        ...
        assert result[-1] == ">"
        return f"{result[:-1]} (soft){result[-1]}"
...
class CPointerBaseType(CType):
    ...
    def literal_code(self, value):
        if self.is_string:
            ...
            return f'"{StringEncoding.escape_byte_string(value)}"'
        return str(value)
...
class CArrayType(CPointerBaseType):
    ...
    def __repr__(self):
        return f"<CArrayType {self.size} {repr(self.base_type)}>"
    ...
    def create_to_py_utility_code(self, env):
        ...
        to_py_function = f"__Pyx_carray_to_py_{safe_typename}"
        to_tuple_function = f"__Pyx_carray_to_tuple_{safe_typename}"
    ...
    def to_py_call_code(self, source_code, result_code, result_type, to_py_function=None):
        func = self.to_py_function if to_py_function is None else to_py_function
        if self.is_string or self.is_pyunicode_ptr:
            return f'{result_code} = {func}({source_code})'
    ...
    def create_from_py_utility_code(self, env):
        ...
        from_py_function = f"__Pyx_carray_from_py_{self.base_type.specialization_name()}"
    ...
    def from_py_call_code(self, source_code, result_code, error_pos, code,
                          from_py_function=None, error_condition=None,
                          special_none_cvalue=None):
        assert not error_condition, f'{error_pos}: {error_condition}'
        assert not special_none_cvalue, f'{error_pos}: {special_none_cvalue}'  # not currently supported
        call_code = "{}({}, {}, {})".format(
            from_py_function or self.from_py_function,
...
class CPtrType(CPointerBaseType):
    ...
    def __repr__(self):
        return f"<CPtrType {repr(self.base_type)}>"
...
class CReferenceBaseType(BaseType):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__!r} {self.ref_base_type}>"
...
class CFakeReferenceType(CReferenceType):
    ...
    def declaration_code(self, entity_code,
            for_display = 0, dll_linkage = None, pyrex = 0):
        #print "CReferenceType.declaration_code: pointer to", self.base_type ###
        return f"__Pyx_FakeReference<{self.ref_base_type.empty_declaration_code()}> {entity_code}"
...
class CFuncType(CType):
    ...
    def __repr__(self):
        ...
        if self.has_varargs:
            arg_reprs.append("...")
        if self.exception_value:
            except_clause = f" {self.exception_value!r}"
        else:
            except_clause = ""
        if self.exception_check:
            except_clause += "?"
    ...
    def declaration_code(self, entity_code,
        ...
        if self.is_overridable:
            arg_decl_list.append(f"int {Naming.skip_dispatch_cname}")
        ...
        if (pyrex or for_display) and not self.return_type.is_pyobject:
            if self.exception_value and self.exception_check:
                trailer = f" except? {self.exception_value}"
        ...
        if not with_calling_convention:
            ...
            if (not entity_code and cc) or entity_code.startswith("*"):
                entity_code = f"({cc}{entity_code})"
                cc = ""
        if self.is_const_method:
            trailer += " const"
        return self.return_type.declaration_code(
            f"{cc}{entity_code}({arg_decl_code}){trailer}",
            for_display, dll_linkage, pyrex)
    ...
    def function_header_code(self, func_name, arg_code):
        if self.is_const_method:
            ...
            trailer = ""
        return f"{self.calling_convention_prefix()}{func_name}({arg_code}){trailer}"
    ...
    def create_to_py_utility_code(self, env):
        ...
        def arg_name_part(arg):
            return f"{len(arg.name)}{punycodify_name(arg.name)}" if arg.name else "0"
        arg_names = [ arg_name_part(arg) for arg in self.args ]
        arg_names = "_".join(arg_names)
        safe_typename = type_identifier(self, pyrex=True)
        to_py_function = f"__Pyx_CFunc_{safe_typename}_to_py_{arg_names}"
        ...
        context = {
            'cname': to_py_function,
            'args': [Arg(arg.name or f'arg{ix}', arg.type) for ix, arg in enumerate(self.args)],
    ...
    if entry.is_cmethod:
        ...
        if entry.is_inherited:
            entry.cname = StringEncoding.EncodedString(
                    f"{Naming.obj_base_cname}.{entry.cname}")
    else:
...
def get_fused_cname(fused_cname, orig_cname):
    ...
    return StringEncoding.EncodedString('{}{}{}'.format(Naming.fused_func_prefix,
...
class ToPyStructUtilityCode:
    ...
    def __init__(self, type, forward_decl, env):
        self.type = type
        self.header = "static PyObject* {}({})".format(type.to_py_function,
    ...
    def put_code(self, output):
        ...
        for member in self.type.scope.var_entries:
            ...
            code.putln(f"if (unlikely(PyDict_SetItem(res, {nameconst_cname}, member) < 0)) goto bad;")
...
class CStructOrUnionType(CType):
    ...
    def __init__(self, name, kind, scope, typedef_flag, cname, packed=False, in_cpp=False):
        ...
        self.to_py_function = "{}_to_py_{}".format(
            Naming.convert_func_prefix, self.specialization_name())
        self.from_py_function = "{}_from_py_{}".format(
            Naming.convert_func_prefix, self.specialization_name())
    ...
    def __repr__(self):
        return "<CStructOrUnionType {} {}{}>".format(
    ...
    def declaration_code(self, entity_code,
        ...
        if pyrex or for_display:
            ...
            if self.typedef_flag:
                ...
                base_code = f"{self.kind} {self.cname}"
...
class CppClassType(CType):
    ...
    def create_from_py_utility_code(self, env):
        ...
        if self.cname in builtin_cpp_conversions or self.cname in cpp_string_conversions:
            ...
            if self.cname in cpp_string_conversions:
                ...
                cls = self.cname[5:]
            cname = f"__pyx_convert_{cls}_from_py_{'__and_'.join(tags)}"
            context.update({
                'cname': cname,
    ...
    def create_to_py_utility_code(self, env):
        ...
        if self.cname in builtin_cpp_conversions or self.cname in cpp_string_conversions:
            ...
            if self.cname in cpp_string_conversions:
                ...
                cls = self.cname[5:]
                prefix = ''
            cname = f"__pyx_convert_{prefix}{cls}_to_py_{'____'.join(tags)}"
            context.update({
                'cname': cname,
                'maybe_unordered': self.maybe_unordered(),
                'type': self.cname,
    ...
    def specialize_here(self, pos, env, template_values=None):
        if not self.is_template_type():
            error(pos, f"'{self}' type is not a template")
            return error_type
        if len(self.templates) - self.num_optional_templates <= len(template_values) < len(self.templates):
            ...
            template_values = template_values + [
                TemplatePlaceholderType(
                    f"{partial_specialization}::{param.name}", True)
        ...
        for value in template_values:
            if value.is_pyobject or value.needs_refcounting:
                ...
                error(pos,
                      "{} type '{}' cannot be used as a template argument".format(
    ...
    def declaration_code(self, entity_code,
        ...
        if self.templates:
            ...
            templates = ""
        if pyrex or for_display:
            base_code = f"{self.name}{templates}"
        else:
            base_code = f"{self.cname}{templates}"
            if self.namespace is not None:
                base_code = f"{self.namespace.empty_declaration_code()}::{base_code}"
            base_code = public_decl(base_code, dll_linkage)
        return self.base_declaration_code(base_code, entity_code)

    def cpp_optional_declaration_code(self, entity_code, dll_linkage=None, template_params=None):
        return "__Pyx_Optional_Type<{}> {}".format(
    ...
    def check_nullary_constructor(self, pos, msg="stack allocated"):
        ...
        if constructor is not None and best_match([], constructor.all_alternatives()) is None:
            error(pos, f"C++ class must have a nullary constructor to be {msg}")
...
class EnumMixin:
    ...
    def create_enum_to_py_utility_code(self, env):
        from .UtilityCode import CythonUtilityCode
        self.to_py_function = f"__Pyx_Enum_{self.name}_to_py"
...
class CppScopedEnumType(CType, EnumMixin):
    ...
    def declaration_code(self, entity_code,
        ...
        if pyrex or for_display:
            ...
            if self.namespace:
                type_name = f"{self.namespace.empty_declaration_code()}::{self.cname}"
            else:
                type_name = f"__PYX_ENUM_CLASS_DECL {self.cname}"
    ...
    def create_to_py_utility_code(self, env):
        ...
        if self.underlying_type.create_to_py_utility_code(env):
            ...
            self.to_py_function = '[](const {}& x){{return {}(({})x);}}'.format(
                self.cname,
...
class CEnumType(CIntLike, CType, EnumMixin):
    ...
    def __init__(self, name, cname, typedef_flag, namespace=None, doc=None):
        ...
        self.typedef_flag = typedef_flag
        self.namespace = namespace
        self.default_value = f"({self.empty_declaration_code()}) 0"

    def __str__(self):
        return self.name

    def __repr__(self):
        return "<CEnumType {} {}{}>".format(self.name, self.cname,
            ("", " typedef")[self.typedef_flag])
    ...
    def declaration_code(self, entity_code,
        ...
        if pyrex or for_display:
            ...
            if self.namespace:
                base_code = f"{self.namespace.empty_declaration_code()}::{self.cname}"
            elif self.typedef_flag:
                base_code = self.cname
            else:
                base_code = f"enum {self.cname}"
            base_code = public_decl(base_code, dll_linkage)
        return self.base_declaration_code(base_code, entity_code)
...
class PythonTypeConstructorMixin:
    ...
    def __repr__(self):
        if self.base_type:
            return f"{self.name}[{self.base_type!r}]"
        else:
            return self.name
...
class SpecialPythonTypeConstructor(PyObjectType, PythonTypeConstructorMixin):
    ...
    def specialize_here(self, pos, env, template_values=None):
        if len(template_values) != 1:
            error(pos, f"'{self.name}' takes exactly one template argument.")
            return error_type
    ...
    for func in functions:
        ...
        if func_type.templates:
            ...
            if deductions is None:
                errors.append((func, "Unable to deduce type parameters for {} given ({})".format(
                ...
                specialization = Entry(
                    name = func.name + f"[{','.join([str(t) for t in type_list])}]",
                    cname = func.cname + f"<{','.join([t.empty_declaration_code() for t in type_list])}>",
...
_escape_special_type_characters = partial(re.compile(
    ...
    f" ?({'|'.join(re.escape(s) for s in sorted(_special_type_characters, reverse=True))}) ?"
...
def type_identifier_from_declaration(decl):
    ...
    if safe is None:
        ...
        safe = cap_length(re.sub('[^a-zA-Z0-9_]', lambda x: f'__{ord(x.group(0)):X}', safe))
...
def cap_length(s, max_prefix=63, max_len=1024):
    ...
    return f'{hash_prefix}__{s[:max_len-17]}__etc'

def write_noexcept_performance_hint(pos, env, function_name=None, void_return=False):
    on_what = f"on '{function_name}' " if function_name else ""
    msg = f"Exception check {on_what}will always require the GIL to be acquired."
    ...
    if len(solutions) == 1:
        msg = f"{msg} {solutions[0]}"
    else:
        solutions = [f"\t{i+1}. {s}" for i, s in enumerate(solutions)]
        msg = "{}\nPossible solutions:\n{}".format(msg, "\n".join(solutions))
