...
def create_cython_argparser():
    ...
    parser.add_argument(
        "-l", "--create-listing", dest="use_listing_file", action="store_const", const=1, help="Write error messages to a listing file"
    )
    parser.add_argument(
        "-I",
        "--include-dir",
    ...
    parser.add_argument(
        ...
        action="store",
        type=int,
        help="Release interned objects on python exit, for memory debugging. " "Level indicates aggressiveness, default 0 releases nothing.",
    )
    parser.add_argument(
        "-w",
        "--working",
    ...
    parser.add_argument(
        "--gdb-outdir", action=SetGDBDebugOutputAction, type=str, help="Specify gdb debug information output directory. Implies --gdb."
    )
    ...
    parser.add_argument(
        ...
        action="store_const",
        const="main",
        help="Generate a main() function that embeds the Python interpreter. " "Pass --embed=<method_name> for a name other than main().",
    )
    ...
    parser.add_argument(
        ...
        action="store_const",
        const="3str",
        help="Compile based on Python-3 syntax and code semantics without " "assuming unicode by default for string literals under Python 2.",
    )
    parser.add_argument(
        "--lenient", action=SetLenientAction, nargs=0, help="Change some compile time errors to runtime errors to " "improve Python compatibility"
    )
    parser.add_argument(
