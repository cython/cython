...
class IterationTransform(Visitor.EnvTransform):
    ...
    def visit_PrimaryCmpNode(self, node):
        if node.is_ptr_contains():
            ...
            if_body = Nodes.StatListNode(
                pos, stats=[Nodes.SingleAssignmentNode(pos, lhs=result_ref, rhs=ExprNodes.BoolNode(pos, value=1)), Nodes.BreakStatNode(pos)]
            )
    ...
    def _optimise_for_loop(self, node, iterable, reversed=False):
        ...
        if (
            ...
            and 1 <= arg_count <= 3
            and (iterable.self is None and function.is_name and function.name in ("range", "xrange") and function.entry and function.entry.is_builtin)
        ):
    ...
    def _transform_bytes_iteration(self, node, slice_node, reversed=False):
        ...
        slice_base_node = ExprNodes.PythonCapiCallNode(
            slice_node.pos, "PyBytes_AS_STRING", self.PyBytes_AS_STRING_func_type, args=[unpack_temp_node], is_temp=0
        )
        len_node = ExprNodes.PythonCapiCallNode(
            slice_node.pos, "PyBytes_GET_SIZE", self.PyBytes_GET_SIZE_func_type, args=[unpack_temp_node], is_temp=0
        )
        ...
        return UtilNodes.LetNode(
            ...
            self._transform_carray_iteration(
                ...
                ExprNodes.SliceIndexNode(
                    slice_node.pos, base=slice_base_node, start=None, step=None, stop=len_node, type=slice_base_node.type, is_temp=1
                ),
                reversed=reversed,
            ),
        )
    ...
    def _transform_unicode_iteration(self, node, slice_node, reversed=False):
            ...
            try:
                ...
                bytes_slice = ExprNodes.SliceIndexNode(
                    ...
                    base=ExprNodes.BytesNode(
                        slice_node.pos, value=bytes_value, constant_result=bytes_value, type=PyrexTypes.c_const_char_ptr_type
                    ).coerce_to(PyrexTypes.c_const_uchar_ptr_type, self.current_env()),
                    start=None,
                    stop=ExprNodes.IntNode(
                        slice_node.pos, value=str(len(bytes_value)), constant_result=len(bytes_value), type=PyrexTypes.c_py_ssize_t_type
                    ),
        ...
        setup_node = Nodes.ExprStatNode(
            ...
            expr=ExprNodes.PythonCapiCallNode(
                ...
                args=[
                    unpack_temp_node,
                    ExprNodes.AmpersandNode(slice_node.pos, operand=length_temp.ref(slice_node.pos), type=PyrexTypes.c_py_ssize_t_ptr_type),
                    ExprNodes.AmpersandNode(slice_node.pos, operand=data_temp.ref(slice_node.pos), type=PyrexTypes.c_void_ptr_ptr_type),
                    ExprNodes.AmpersandNode(slice_node.pos, operand=kind_temp.ref(slice_node.pos), type=PyrexTypes.c_int_ptr_type),
                ],
                is_temp=True,
        ...
        return UtilNodes.LetNode(
            ...
            UtilNodes.TempsBlockNode(
                node.pos, temps=[counter_temp, length_temp, data_temp, kind_temp], body=Nodes.StatListNode(node.pos, stats=[setup_node, loop_node])
            ),
        )
    ...
    def _transform_carray_iteration(self, node, slice_node, reversed=False):
        ...
        if isinstance(slice_node, ExprNodes.SliceIndexNode):
            ...
            start = None
            stop = ExprNodes.IntNode(
                slice_node.pos, value=str(slice_node.type.size), type=PyrexTypes.c_py_ssize_t_type, constant_result=slice_node.type.size
            )
            step = None
        ...
        if slice_base.type.is_string and node.target.type.is_pyobject:
            ...
            if slice_node.type is Builtin.unicode_type:
                target_value = ExprNodes.CastNode(
                    ExprNodes.DereferenceNode(node.target.pos, operand=counter_temp, type=ptr_type.base_type), PyrexTypes.c_py_ucs4_type
            ...
            if abs_step != 1:
                if isinstance(bound1.constant_result, _py_int_types) and isinstance(bound2.constant_result, _py_int_types):
                    ...
                    bound1 = ExprNodes.IntNode(
                        bound1.pos, value=str(bound1_value), constant_result=bound1_value, type=PyrexTypes.spanning_type(bound1.type, bound2.type)
                    )
                else:
        ...
        step_calculation_node = ExprNodes.binop_node(
            ...
            operand1=ExprNodes.binop_node(
                ...
                operand2=ExprNodes.MulNode(
                    bound1.pos,
                    operand1=ExprNodes.IntNode(bound1.pos, value=str(abs_step), constant_result=abs_step, type=spanning_step_type),
                    operator="*",
                    operand2=ExprNodes.DivNode(
                        bound1.pos,
                        operand1=ExprNodes.SubNode(
                            bound1.pos,
                            operand1=ExprNodes.SubNode(bound1.pos, operand1=begin_value, operator="-", operand2=end_value, type=spanning_type),
                            operator="-",
                            operand2=ExprNodes.IntNode(bound1.pos, value="1", constant_result=1),
                            type=spanning_step_type,
                        ),
                        operator="//",
                        operand2=ExprNodes.IntNode(bound1.pos, value=str(abs_step), constant_result=abs_step, type=spanning_step_type),
                        type=spanning_step_type,
                    ),
                    type=spanning_step_type,
                ),
    ...
    def _transform_dict_iteration(self, node, dict_obj, method, keys, values):
        ...
        result_code = [
            Nodes.SingleAssignmentNode(node.pos, lhs=pos_temp, rhs=ExprNodes.IntNode(node.pos, value="0", constant_result=0)),
            Nodes.SingleAssignmentNode(
                ...
                rhs=ExprNodes.PythonCapiCallNode(
                    ...
                    args=[dict_obj, flag_node(dict_obj.type is Builtin.dict_type), method_node, dict_len_temp_addr, is_dict_temp_addr],
                    is_temp=True,
                ),
            ),
        ...
        result_code = [
            ...
            Nodes.SingleAssignmentNode(
                ...
                rhs=ExprNodes.PythonCapiCallNode(
                    ...
                    args=[set_obj, flag_node(set_obj.type is Builtin.set_type), set_len_temp_addr, is_set_temp_addr],
                    is_temp=True,
                ),
            ),
            Nodes.WhileStatNode(node.pos, condition=None, body=body, else_clause=node.else_clause),
...
class EarlyReplaceBuiltinCalls(Visitor.EnvTransform):
        ...
        if isinstance(arg, (ExprNodes.UnicodeNode, ExprNodes.BytesNode)):
            ...
            if arg.unicode_value and len(arg.unicode_value) == 1 and ord(arg.unicode_value) <= 255:  # Py2/3 portability
                return ExprNodes.IntNode(
                    arg.pos, type=PyrexTypes.c_int_type, value=str(ord(arg.unicode_value)), constant_result=ord(arg.unicode_value)
                )
        return node
        ...
        test_node = Nodes.IfStatNode(
            ...
            if_clauses=[
                Nodes.IfClauseNode(
                    yield_expression.pos,
                    condition=condition,
                    body=Nodes.ReturnStatNode(node.pos, value=ExprNodes.BoolNode(yield_expression.pos, value=is_any, constant_result=is_any)),
                )
            ],
        )
    ...
    def _handle_simple_function_sorted(self, node, pos_args):
        ...
        if isinstance(arg, ExprNodes.ComprehensionNode) and arg.type is Builtin.list_type:
            ...
            list_node = loop_node = ExprNodes.PythonCapiCallNode(
                node.pos,
                "__Pyx_PySequence_ListKeepNew" if arg.is_temp and arg.type in (PyrexTypes.py_object_type, Builtin.list_type) else "PySequence_List",
    ...
    def __handle_simple_function_sum(self, node, pos_args):
        ...
        exec_code = Nodes.StatListNode(
            node.pos,
            stats=[
                Nodes.SingleAssignmentNode(start.pos, lhs=UtilNodes.ResultRefNode(pos=node.pos, expression=result_ref), rhs=start, first=True),
                loop_node,
            ],
        )
    ...
    def _optimise_min_max(self, node, args, operator):
        ...
        for arg_node in cascaded_nodes:
            ...
            last_result = ExprNodes.CondExprNode(
                ...
                test=ExprNodes.PrimaryCmpNode(arg_node.pos, operand1=arg_node, operator=operator, operand2=result_ref),
            )
    ...
    def _transform_list_set_genexpr(self, node, pos_args, target_type):
        ...
        result_node = ExprNodes.InlinedGeneratorExpressionNode(
            node.pos, gen_expr_node, orig_func="set" if target_type is Builtin.set_type else "list", comprehension_type=target_type
        )
    ...
    def _handle_simple_function_dict(self, node, pos_args):
        ...
        for yield_expression, yield_stat_node in yield_statements:
            append_node = ExprNodes.DictComprehensionAppendNode(
                yield_expression.pos, key_expr=yield_expression.args[0], value_expr=yield_expression.args[1], target=result_node.target
            )
            Visitor.recursively_replace_node(gen_expr_node, yield_stat_node, append_node)
...
class InlineDefNodeCalls(Visitor.NodeRefCleanupMixin, Visitor.EnvTransform):
    ...
    def visit_SimpleCallNode(self, node):
        ...
        inlined = ExprNodes.InlinedDefNodeCallNode(
            node.pos, function_name=function_name, function=function, args=node.args, generator_arg_tag=node.generator_arg_tag
        )
...
class OptimizeBuiltinCalls(Visitor.NodeRefCleanupMixin, Visitor.MethodDispatcherTransform):
    ...
    PyObject_String_func_type = PyrexTypes.CFuncType(
        PyrexTypes.py_object_type,
        [PyrexTypes.CFuncTypeArg("obj", PyrexTypes.py_object_type, None)],  # Change this to Builtin.str_type when removing Py2 support.
    )
    ...
    def _handle_simple_function_str(self, node, function, pos_args):
        ...
        return ExprNodes.PythonCapiCallNode(
            node.pos, cname, self.PyObject_String_func_type, args=pos_args, is_temp=node.is_temp, utility_code=utility_code, py_name="str"
        )
    ...
    def _handle_simple_function_unicode(self, node, function, pos_args):
        ...
        return ExprNodes.PythonCapiCallNode(
            node.pos, cname, self.PyObject_Unicode_func_type, args=pos_args, is_temp=node.is_temp, utility_code=utility_code, py_name="unicode"
        )
    ...
    def _handle_simple_function_set(self, node, function, pos_args):
        ...
        if pos_args[0].is_sequence_constructor:
            ...
            return self.replace(
                node,
                ExprNodes.PythonCapiCallNode(node.pos, "PySet_New", self.PySet_New_func_type, args=pos_args, is_temp=node.is_temp, py_name="set"),
            )
    ...
    def _handle_simple_function_float(self, node, function, pos_args):
        ...
        if arg is None:
            arg = func_arg.as_none_safe_node("float() argument must be a string or a number, not 'NoneType'")
    ...
    Pyx_strlen_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_size_t_type, [PyrexTypes.CFuncTypeArg("bytes", PyrexTypes.c_const_char_ptr_type, None)], nogil=True
    )

    Pyx_ssize_strlen_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_py_ssize_t_type, [PyrexTypes.CFuncTypeArg("bytes", PyrexTypes.c_const_char_ptr_type, None)], exception_value="-1"
    )

    Pyx_Py_UNICODE_strlen_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_py_ssize_t_type, [PyrexTypes.CFuncTypeArg("unicode", PyrexTypes.c_const_py_unicode_ptr_type, None)], exception_value="-1"
    )

    PyObject_Size_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_py_ssize_t_type, [PyrexTypes.CFuncTypeArg("obj", PyrexTypes.py_object_type, None)], exception_value="-1"
    )
        ...
        for test_type_node in types:
            ...
            test_nodes.append(
                ExprNodes.PythonCapiCallNode(
                    test_type_node.pos, type_check_function, self.Py_type_check_func_type, args=type_check_args, is_temp=True
                )
            )
    ...
    def _handle_simple_function_ord(self, node, function, pos_args):
        ...
        if isinstance(arg, ExprNodes.CoerceToPyTypeNode):
            ...
            if len(arg.value) == 1:
                return ExprNodes.IntNode(arg.pos, type=PyrexTypes.c_int_type, value=str(ord(arg.value)), constant_result=ord(arg.value)).coerce_to(
    ...
    Pyx_tp_new_func_type = PyrexTypes.CFuncType(
        PyrexTypes.py_object_type,
        [PyrexTypes.CFuncTypeArg("type", PyrexTypes.py_object_type, None), PyrexTypes.CFuncTypeArg("args", Builtin.tuple_type, None)],
    )
    ...
    def _handle_any_slot__new__(self, node, function, args, is_unbound_method, kwargs=None):
        ...
        if kwargs:
            ...
            return ExprNodes.PythonCapiCallNode(
                node.pos, "__Pyx_tp_new", self.Pyx_tp_new_func_type, args=[type_arg, args_tuple], utility_code=utility_code, is_temp=node.is_temp
            )
    ...
    PyObject_Append_func_type = PyrexTypes.CFuncType(
        ...
        [PyrexTypes.CFuncTypeArg("list", PyrexTypes.py_object_type, None), PyrexTypes.CFuncTypeArg("item", PyrexTypes.py_object_type, None)],
        exception_value="-1",
    )
    ...
    PyByteArray_Append_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_returncode_type,
        [PyrexTypes.CFuncTypeArg("bytearray", PyrexTypes.py_object_type, None), PyrexTypes.CFuncTypeArg("value", PyrexTypes.c_int_type, None)],
        exception_value="-1",
    )

    PyByteArray_AppendObject_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_returncode_type,
        [PyrexTypes.CFuncTypeArg("bytearray", PyrexTypes.py_object_type, None), PyrexTypes.CFuncTypeArg("value", PyrexTypes.py_object_type, None)],
        exception_value="-1",
    )
    ...
    def _handle_simple_method_bytearray_append(self, node, function, args, is_unbound_method):
        ...
        new_node = ExprNodes.PythonCapiCallNode(
            node.pos, func_name, func_type, args=[args[0], value], may_return_none=False, is_temp=node.is_temp, utility_code=utility_code
        )
        ...
        if len(args) == 1:
            ...
            return ExprNodes.PythonCapiCallNode(
                ...
                args=[
                    ...
                    ExprNodes.RawCNameExprNode(index.pos, PyrexTypes.c_void_type, orig_index_type.empty_declaration_code()),
    ...
    single_param_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_returncode_type, [PyrexTypes.CFuncTypeArg("obj", PyrexTypes.py_object_type, None)], exception_value="-1"
    )
    ...
    def _handle_simple_method_list_sort(self, node, function, args, is_unbound_method):
        ...
        if len(args) != 1:
            return node
        return self._substitute_method_call(node, function, "PyList_Sort", self.single_param_func_type, "sort", is_unbound_method, args).coerce_to(
            node.type, self.current_env
        )
    ...
    def _optimise_num_binop(self, operator, node, function, args, is_unbound_method):
        ...
        call_node = self._substitute_method_call(
            node,
            function,
            "__Pyx_Py%s_%s%s%s" % ("Float" if is_float else "Int", "" if ret_type.is_pyobject else "Bool", operator, arg_order),
    ...
    PyUnicode_uchar_predicate_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_bint_type, [PyrexTypes.CFuncTypeArg("uchar", PyrexTypes.c_py_ucs4_type, None)]
    )
    ...
    PyUnicode_uchar_conversion_func_type = PyrexTypes.CFuncType(
        PyrexTypes.c_py_ucs4_type, [PyrexTypes.CFuncTypeArg("uchar", PyrexTypes.c_py_ucs4_type, None)]
    )
    ...
    PyUnicode_Splitlines_func_type = PyrexTypes.CFuncType(
        Builtin.list_type,
        [PyrexTypes.CFuncTypeArg("str", Builtin.unicode_type, None), PyrexTypes.CFuncTypeArg("keepends", PyrexTypes.c_bint_type, None)],
    )
    ...
    def _handle_simple_method_unicode_splitlines(self, node, function, args, is_unbound_method):
        ...
        return self._substitute_method_call(
            node, function, "PyUnicode_Splitlines", self.PyUnicode_Splitlines_func_type, "splitlines", is_unbound_method, args
        )
    ...
    PyUnicode_Join_func_type = PyrexTypes.CFuncType(
        ...
        [PyrexTypes.CFuncTypeArg("str", Builtin.unicode_type, None), PyrexTypes.CFuncTypeArg("seq", PyrexTypes.py_object_type, None)],
    )
    ...
    def _handle_simple_method_unicode_encode(self, node, function, args, is_unbound_method):
        ...
        if encoding and error_handling == "strict":
            ...
            if codec_name is not None and "-" not in codec_name:
                ...
                return self._substitute_method_call(
                    node, function, encode_function, self.PyUnicode_AsXyzString_func_type, "encode", is_unbound_method, [string_node]
                )
        ...
        if string_type in (Builtin.bytes_type, Builtin.bytearray_type):
            if is_unbound_method:
                string_node = string_node.as_none_safe_node(
                    "descriptor '%s' requires a '%s' object but received a 'NoneType'", format_args=["decode", string_type.name]
                )
            else:
                string_node = string_node.as_none_safe_node(
    ...
    def _wrap_self_arg(self, self_arg, function, is_unbound_method, attr_name):
        ...
        if is_unbound_method:
            self_arg = self_arg.as_none_safe_node(
                "descriptor '%s' requires a '%s' object but received a 'NoneType'", format_args=[attr_name, self_arg.type.name]
            )
        else:
            self_arg = self_arg.as_none_safe_node(
    ...
    def _inject_null_for_none(self, args, index):
        ...
        args[index] = (
            ...
            if arg.is_none
                ...
                arg.pos, "__Pyx_NoneAsNull", self.obj_to_obj_func_type, args=[arg.coerce_to_simple(self.current_env())], is_temp=0
            )
        )
...
class ConstantFolding(Visitor.VisitorTransform, SkipDeclarations):
    ...
    def visit_UnopNode(self, node):
        ...
        if node.operator == "!":
            ...
            return ExprNodes.IntNode(
                node.pos, value=str(int(node.constant_result)), type=PyrexTypes.c_int_type, constant_result=int(node.constant_result)
            )
        elif node.operator == "+":
    ...
    def _handle_UnaryMinusNode(self, node):
        ...
        if node_type.is_int and node_type.signed or isinstance(node.operand, ExprNodes.IntNode) and node_type.is_pyobject:
            return ExprNodes.IntNode(
                node.pos, value=_negate(node.operand.value), type=node_type, longness=node.operand.longness, constant_result=node.constant_result
            )
        return node
    ...
    def visit_BinopNode(self, node):
        ...
        if target_class is ExprNodes.IntNode:
            ...
            new_node = ExprNodes.IntNode(
                pos=node.pos, unsigned=unsigned, longness=longness, value=str(int(node.constant_result)), constant_result=int(node.constant_result)
            )
            ...
            if format_type in "asrfdoxX":
                ...
                substrings.append(
                    ExprNodes.FormattedValueNode(
                        ...
                        format_spec=ExprNodes.UnicodeNode(pos, value=EncodedString(format_spec), constant_result=format_spec)
                        if format_spec
                        else None,
                    )
                )
    ...
    def visit_PrimaryCmpNode(self, node):
        ...
        for cascade in cascades:
            ...
            pcmp_node = ExprNodes.PrimaryCmpNode(
                cmp_node.pos, operand1=cascade[0], operator=cmp_node.operator, operand2=cmp_node.operand2, constant_result=not_a_constant
            )
            ...
            if isinstance(node.arg_tuple, ExprNodes.TupleNode) and not (
                ...
                if function.type is Builtin.type_type:
                    ...
                    if entry.is_builtin or entry.type.is_cfunction:
                        ...
                        may_be_a_method = any(
                            assignment.rhs and not isinstance(assignment.rhs, non_method_nodes) for assignment in entry.cf_assignments
                        )
                if may_be_a_method:
