...
class IterationTransform(Visitor.EnvTransform):
    ...
    def _try_optimise_iterator_function(self, node, iterable, reversed):
        ...
        # range() iteration?
        if Options.options_in_thread.convert_range and 1 <= arg_count <= 3 and (
    ...
    def _transform_indexable_iteration(self, node, slice_node, is_mutable, reversed=False):
        ...
        env = self.current_env()
        new_directives = Options.options_in_thread.copy_inherited_directives(env.directives, boundscheck=False, wraparound=False)
