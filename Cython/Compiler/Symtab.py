...
class Scope(object):
    ...
    def declare_cfunction(
        self, name, type, pos, cname=None, visibility="private", api=0, in_pxd=0, defining=0, modifiers=(), utility_code=None, overridable=False
    ):
    ...
    def lookup(self, name):
        ...
        entry = self.lookup_here(name) or (self.outer_scope and self.outer_scope.lookup(mangled_name)) or None  # lookup here also does mangling
        if entry:
            return entry
...
class ModuleScope(Scope):
    ...
    def __init__(self, name, parent_module, context, is_package=False):
        ...
        for var_name in ["__builtins__", "__name__", "__file__", "__doc__", "__path__", "__spec__", "__loader__", "__package__", "__cached__"]:
    ...
    def find_module(self, module_name, pos, relative_level=-1):
        ...
        return module_scope.context.find_module(module_name, relative_to=relative_to, pos=pos, absolute_fallback=absolute_fallback)
    ...
    def declare_var(self, name, type, pos, cname=None, visibility="private", api=False, in_pxd=False, is_cdef=False, pytyping_modifiers=None):
        ...
        entry = Scope.declare_var(
            self, name, type, pos, cname=cname, visibility=visibility, api=api, in_pxd=in_pxd, is_cdef=is_cdef, pytyping_modifiers=pytyping_modifiers
        )
        if is_cdef:
            entry.is_cglobal = 1
    ...
    def declare_c_class(
        ...
        if not type.scope:
            if defining or implementing:
                scope = CClassScope(name=name, outer_scope=self, visibility=visibility)
...
class StructOrUnionScope(Scope):
    ...
    def declare_cfunction(
        self, name, type, pos, cname=None, visibility="private", api=0, in_pxd=0, defining=0, modifiers=(), overridable=False
    ...
    def declare_inherited_c_attributes(self, base_scope):
        ...
        for base_entry in base_scope.cfunc_entries:
            ...
            entry = self.add_cfunction(
                base_entry.name, base_entry.type, base_entry.pos, cname, base_entry.visibility, base_entry.func_modifiers, inherited=True
            )
...
class CppClassScope(Scope):
    ...
    def declare_var(
        self, name, type, pos, cname=None, visibility="extern", api=False, in_pxd=False, is_cdef=False, defining=False, pytyping_modifiers=None
    ):
        # Add an entry for an attribute.
        if not cname:
            cname = name
    ...
    def declare_cfunction(
        self, name, type, pos, cname=None, visibility="extern", api=0, in_pxd=0, defining=0, modifiers=(), utility_code=None, overridable=False
    ):
