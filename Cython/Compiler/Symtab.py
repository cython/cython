...
class PreImportScope(Scope):
    ...
    def __init__(self):
        Scope.__init__(self, Options.optionsInThread.pre_import, None, None)
...
class BuiltinScope(Scope):
    ...
    def __init__(self):
        if Options.optionsInThread.pre_import is None:
    ...
    def declare_builtin(self, name, pos):
        if name not in PyrexTypes.KNOWN_PYTHON_BUILTINS:
            if self.outer_scope is not None:
                ...
                if Options.optionsInThread.error_on_unknown_names:
    ...
    def declare_builtin_type(self, name, cname,
        ...
        if Options.optionsInThread.cache_builtins:
...
class ModuleScope(Scope):
    ...
    def declare_builtin(self, name, pos):
        ...
        if Options.optionsInThread.cache_builtins and name not in PyrexTypes.uncachable_builtins:
            entry.is_builtin = 1
    ...
    def declare_var(self, name, type, pos,
        ...
        if is_cdef:
            ...
            entry.is_pyglobal = 1
        if Options.optionsInThread.cimport_from_pyx:
            entry.used = 1
        return entry
