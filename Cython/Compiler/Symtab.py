            ...
            if entry.type.same_as(type):
                ...
                if visibility == 'extern' and entry.visibility == 'extern':
                    ...
                    if can_override:
                      ...
                      and (entry.defined_in_pxd or getattr(entry, 'inline_func_in_pxd', False))
                      and type.compatible_signature_with(entry.type)):
...
class PropertyScope(Scope):
    ...
    def declare_cfunction(self, name, type, pos, *args, **kwargs):
        ...
        if name=="__set__":
            if not (type.return_type.is_void or getattr(type.return_type, 'is_returncode', False)):
                error(pos, "C property setter must return 'void' or int return code")
            if len(type.args) != 2:
