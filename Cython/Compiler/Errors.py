...
def context(position):
    source = position[0]
    assert not (isinstance(source, any_string_type)), "Please replace filename strings with Scanning.FileSourceDescriptor instances %r" % source
    try:
        F = source.get_lines()
...
def report_error(err, use_stack=True):
    ...
    if error_stack and use_stack:
        ...
        try:
            ...
            line = format_error(getattr(err, "message_only", "[unprintable exception message]"), getattr(err, "position", None)) + "\n"
