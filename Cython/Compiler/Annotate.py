...
class AnnotationCCodeWriter(CCodeWriter):
    ...
    _onclick_attr = ' onclick="{0}"'.format(
        ("(function(s){" "    s.display =  s.display === 'block' ? 'none' : 'block'" "})(this.nextElementSibling.style)").replace(
            " ", ""
    ...
    def _save_annotation_header(self, c_file, source_filename, coverage_timestamp=None):
        ...
        if coverage_timestamp:
            coverage_info = " with coverage data from {timestamp}".format(timestamp=datetime.fromtimestamp(int(coverage_timestamp) // 1000))
        ...
        for k, line in enumerate(lines, 1):
            ...
            outlist.append(
                ...
                '{expandsymbol}<span class="{covered}">{line:0{lineno_width}d}</span>: {code}</pre>\n'.format(
                    score=score, expandsymbol=expandsymbol, covered=covered, lineno_width=lineno_width, line=k, code=line.rstrip(), onclick=onclick
                )
            )
            if c_code:
                outlist.append("<pre class='cython code score-{score} {covered}'>{code}</pre>".format(score=score, covered=covered, code=c_code))
        outlist.append("</div>")
