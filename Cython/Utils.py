...
def is_cython_generated_file(path, allow_failed=False, must_exist=True):
    failure_content = b"#error Do not use this file, it is the result of a failed Cython compilation."
    try:
        with open(path, "rb") as f:
            content = f.read(len(failure_content))
    except OSError:
    ...
    return (
        # Cython C file?
        content.startswith(b"/* Generated by Cython ") or
        # Cython output file after previous failures?
        (allow_failed and content == failure_content) or
