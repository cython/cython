...
cdef extern from "Python.h":
    ...
    tuple PyTuple_New(isize len)
    # Return value: New reference.
    # Return a new tuple object of size len, or NULL on failure.

    tuple PyTuple_Pack(isize n, ...)
    # Return value: New reference.
    ...
    isize PyTuple_Size(object  p) except -1
    # Take a pointer to a tuple object, and return the size of that tuple.

    isize PyTuple_GET_SIZE(object  p)
    # Return the size of the tuple p, which must be non-NULL and point
    # to a tuple; no error checking is performed.

    PyObject* PyTuple_GetItem(object  p, isize pos) except NULL
    ...
    PyObject* PyTuple_GET_ITEM(object  p, isize pos)
    # Return value: Borrowed reference.
    # Like PyTuple_GetItem(), but does no checking of its arguments.

    tuple PyTuple_GetSlice(object  p, isize low, isize high)
    # Return value: New reference.
    # Take a slice of the tuple pointed to by p from low to high and return it as a new tuple.

    i32 PyTuple_SetItem(object  p, isize pos, object  o) except -1
    ...
    i32 _PyTuple_Resize(PyObject **p, isize newsize) except -1
