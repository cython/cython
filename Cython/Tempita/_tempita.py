    ...
    def substitute(self, *args, **kw):
        if args:
            ...
            if not hasattr(args[0], "items"):
                raise TypeError(
                    "If you pass in a single argument, you must pass in a dictionary-like object (with a .items() method); you gave %r" % (args[0],)
                )
            kw = args[0]
        ns = kw
...
def lex(s, name=None, trim_whitespace=True, line_offset=0, delimeters=None):
    ...
    token_re = re.compile(r"%s|%s" % (re.escape(delimeters[0]), re.escape(delimeters[1])))
