...
_DataclassParams = namedtuple("_DataclassParams", ["init", "repr", "eq", "order", "unsafe_hash", "frozen"])
...
class Field(object):
    ...
    def __repr__(self):
        return (
            ...
            "compare={7!r},"
            "metadata={8!r},"
            ")".format(self.name, self.type, self.default, self.default_factory, self.init, self.repr, self.hash, self.compare, self.metadata)
        )
...
def dataclass(*args, **kwds):
    raise NotImplementedError("Standard library 'dataclasses' module" "is unavailable, likely due to the version of Python you're using.")
