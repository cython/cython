...
#if PY_VERSION_HEX < 0x030d0000 || (CYTHON_COMPILING_IN_LIMITED_API && !CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING)
...
///////////////////////// GetLazyCachedObject.module_state_decls ////////////////////
...
__pyx_atomic_ptr_type {{object_cname}};
#else
PyObject *{{object_cname}};
#endif
#endif

//////////////////////////// GetLazyCachedObject.module_state_traverse ////////////////////////////

#if {{guard}}
#if (CYTHON_COMPILING_IN_CPYTHON_FREETHREADING || CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING) && CYTHON_ATOMICS
{
    PyObject *value == (PyObject*)__pyx_atomic_pointer_load_acquire(&traverse_module_state->{{object_cname}});
    Py_VISIT(value);
}
#else
Py_VISIT(traverse_module_state->{{object_cname}});
#endif
#endif

//////////////////////////// GetLazyCachedObject.module_state_clear ////////////////////////////

#if {{guard}}
#if (CYTHON_COMPILING_IN_CPYTHON_FREETHREADING || CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING) && CYTHON_ATOMICS
{
    PyObject *value = (PyObject*)__pyx_atomic_pointer_exchange(&clear_module_state->{{object_cname}}, 0);
    Py_XDECREF(value);
}
#else
Py_XDECREF(clear_module_state->{{object_cname}});
clear_module_state->{{object_cname}} = 0;
#endif
#endif

//////////////////////////// GetLazyCachedObject.cleanup ////////////////////////////

#if {{guard}}
#if (CYTHON_COMPILING_IN_CPYTHON_FREETHREADING || CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING) && CYTHON_ATOMICS
{
    PyObject *value = (PyObject*)__pyx_atomic_pointer_exchange(&CGLOBAL({{object_cname}}), 0);
    Py_XDECREF(value);
}
#else
Py_XDECREF(CGLOBAL({{object_cname}}));
CGLOBAL({{object_cname}}) = 0;
#endif
#endif

//////////////////////////// GetLazyCachedObject.proto ///////////////////////////

#if {{guard}}
static PyObject* {{getter_cname}}(void); /* proto */
#endif

/////////////////////////// GetLazyCachedObject //////////////////////////////

// Tempita variables:
// * guard - used to decide if any code is generated
// * object_cname - name of the cached objected in module state
// * getter_cname - the cname of the function being generated by this utility code. It must
//                          appear ahead of this utility code (ie. a proto definition is probably needed)
// * maker_cname - the name of a preexisting function used to generate an owned reference to
//                          an object
//
// The "maker" function may be called more than once (so should be harmless if it is).
// This is most likely in the Limited API where we don't have access to PyMutex.
// However we will always return the result from the first successful completion of the maker
// function.
// The "getter" function produced here returns a borrowed reference (but one controlled by
// Cython so one that will not change).

#if {{guard}}
static PyObject* {{getter_cname}}(void) {
#if PY_VERSION_HEX >= 0x030d0000 && !CYTHON_COMPILING_IN_LIMITED_API
    static PyMutex mutex = {0};
#endif
#if (CYTHON_COMPILING_IN_CPYTHON_FREETHREADING || CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING) && CYTHON_ATOMICS
    PyObject *result = (PyObject*)__pyx_atomic_pointer_load_relaxed(&CGLOBAL({{object_cname}}));
    if (unlikely(!result)) {
        PyObject *existing_result;
#if CYTHON_COMPILING_IN_CPYTHON_FREETHREADING
        PyMutex_Lock(&mutex);
        result = (PyObject*)__pyx_atomic_pointer_load_acquire(&CGLOBAL({{object_cname}}));
        if (likely(!result))
#endif
        {
            result = {{maker_cname}}();
            if (likely(result)) {
                existing_result = (PyObject*)__pyx_atomic_pointer_exchange(&CGLOBAL({{object_cname}}), result);
#if CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING
                if (unlikely(existing_result)) {
                    Py_DECREF(result);
                    result = existing_result;
                }
#else
                (void)existing_result;  // Locked - can't have changed
#endif
            }
        }
#if CYTHON_COMPILING_IN_CPYTHON_FREETHREADING
    ...
    return result;
#else  // atomics unavailable, or not freethreading
    PyObject *result;
#if !(CYTHON_COMPILING_IN_CPYTHON_FREETHREADING || CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING)
    result = CGLOBAL({{object_cname}});
    if (likely(result)) {
        return result;
    }
#endif
#if CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING
    PyObject *arbitrary_object = NAMED_CGLOBAL(moddict_cname);
    Py_BEGIN_CRITICAL_SECTION(arbitrary_object);
...
#endif
    {
        ...
        if (likely(result)) {
#if !CYTHON_COMPILING_IN_CPYTHON_FREETHREADING
            PyObject *existing_result = CGLOBAL({{object_cname}});
            if (unlikely(existing_result)) {
                Py_DECREF(result);
                result = existing_result;
            } else
#endif
            {
                CGLOBAL({{object_cname}}) = result;
            }
        }
    }
#if CYTHON_COMPILING_IN_LIMITED_API_FREETHREADING
    Py_END_CRITICAL_SECTION(arbitrary_object);
#elif PY_VERSION_HEX >= 0x030d0000 && !CYTHON_COMPILING_IN_LIMITED_API
    PyMutex_Unlock(&mutex);
#endif
    return result;
