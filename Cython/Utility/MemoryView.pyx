...
cdef extern from *:
    ...
    bint __PYX_GET_CYTHON_COMPILING_IN_FREETHREADING()
...
cdef class memoryview:
    ...
    def __cinit__(memoryview self, object obj, int flags, bint dtype_is_object=False):
        ...
        if not __PYX_CYTHON_ATOMICS_ENABLED():
            ...
            if (__pyx_memoryview_thread_locks_used < {{THREAD_LOCKS_PREALLOCATED}} and
                    ...
                    not __PYX_GET_CYTHON_COMPILING_IN_FREETHREADING()):
    ...
    def __dealloc__(memoryview self):
        ...
        global __pyx_memoryview_thread_locks_used
        if self.lock != NULL:
            for i in range(0 if __PYX_GET_CYTHON_COMPILING_IN_FREETHREADING() else __pyx_memoryview_thread_locks_used):
...
cdef void slice_copy(memoryview memview, {{memviewslice_name}} *dst) noexcept:
    ...
    dst.memview = <__pyx_memoryview_o *> memview
...
cdef size_t slice_get_size({{memviewslice_name}} *src, int ndim) noexcept nogil:
    ...
    cdef size_t size = <size_t>(<memoryview>(src.memview)).view.itemsize
...
cdef void *copy_data_to_temp({{memviewslice_name}} *src,
    ...
    cdef size_t itemsize = <size_t> (<memoryview>(src.memview)).view.itemsize
