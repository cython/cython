...
pxd_include_dirs = [
    ...
    if "__init__.pyx" in files or "__init__.pxd" in files or directory == os.path.join("Cython", "Includes")
]

pxd_include_patterns = [p + "/*.pxd" for p in pxd_include_dirs] + [p + "/*.pyx" for p in pxd_include_dirs]
...
def compile_cython_modules(profile=False, coverage=False, compile_minimal=False, compile_more=False, cython_with_refnanny=False):
    ...
    pgen = find_executable("pgen", os.pathsep.join([os.environ["PATH"], os.path.join(get_python_inc(), "..", "Parser")]))
    if not pgen:
        ...
        compiled_modules.extend(["Cython.Parser.ConcreteSyntaxTree"])
    ...
    for module in compiled_modules:
        ...
        extensions.append(Extension(module, sources=[pyx_source_file], define_macros=defines if ".refnanny" not in module else [], depends=dep_files))
    ...
    get_directive_defaults().update(language_level=2, binding=False, always_allow_keywords=False, autotestdict=False)
    if profile:
