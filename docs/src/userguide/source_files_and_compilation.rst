...
``CYTHON_CCOMPLEX``
    Turn on to pass complex numbers using the C or C++ language
    standard library types instead of an internal type defined by Cython. Turning
    it on maximizes compatibility with external libraries.  However, MSVC
    ...
    where we thing it's likely to work.
